...
#分页方法，需要传入request和每页需要的文章数
def page_2_page(request, num_of_page):
    all_blogs = Blog.objects.all()
    #分页处理
    paginator = Paginator(all_blogs, num_of_page) #分页处理，num_of_page代表每页的文章数
    #获取页面传来参数（第N页）
...
def blog_list(request):
    all_blogs = Blog.objects.all()
    page_num = page_2_page(request, 4) #调用分页方法，4 - 代表每页的文章数
